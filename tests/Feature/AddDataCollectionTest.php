...
class AddDataCollectionTest extends TestCase
{
    // Trait refresh database agar migration dijalankan
    use RefreshDatabase;

    public function test_user_can_view_create_data_collection()
    {
        ...
        $response = $this->actingAs($user)->get(route('dashboard.fishing-data.data-collection.create', ['id' => $data->dtkn]));
        
        // 3. pengguna berhasil mengakses halaman tambah data ikan
        $response->assertSuccessful();
        $response->assertViewIs('dashboard.fishing-data.data-collection.create');
    }

    public function test_user_cannot_view_create_data_collection()
    {
        ...
        // 2. sebuah akting bahwa pengguna telah melakukan login sebelumnya dan mengakses halaman lihat data data ikan
        $response = $this->actingAs($user)->get(route('dashboard.fishing-data.data-collection.create', ['tid' => $data->dtkn]));
        
        // 3. kondisi dimana dilakukan pengecekkan role apakah pengguna seorang admin atau bukan
        if($user->role == 'admin')
        {
            // 4. jika pengguna adalah seorang admin maka akan diarahkan ke halaman tambah data ikan
            $response->assertSuccessful();
    ...
    public function test_user_can_store_data_data_collection()
    {
        // 1. generate 1 data pengguna dengan role admin
         $user = factory(User::class)->create([
            'name' => 'admin bpp',
        ...
        // 2. sebuah akting bahwa pengguna telah melakukan login sebelumnya dan mengakses halaman tambah data ikan
