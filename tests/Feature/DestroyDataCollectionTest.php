...
class DestroyDataCollectionTest extends TestCase
{
    // Trait refresh database agar migration dijalankan
    use RefreshDatabase;

    public function test_user_can_delete_data_data_collection()
    {
        ...
        // 2. sebuah akting bahwa pengguna telah melakukan login sebelumnya dan mengakses halaman lihat data ikan
        $response = $this->actingAs($user)->get(route('dashboard.fishing-data.data-collection.index', ['id' => $data->dtkn]));
         

        // 4. pengguna melakukan hit url untuk menghapus data ikan
        $this->post(route('dashboard.fishing-data.data-collection.destroy', $data->dtkn));
        
        // 5. proses penghapusan data ikan
        $data->delete();
