...
class DetailDataCollectionTest extends TestCase
{
    // Trait refresh database agar migration dijalankan
    use RefreshDatabase;

    public function test_user_can_detail_data_data_collection()
    {
        ...
        // 2. sebuah akting bahwa pengguna telah melakukan login sebelumnya dan mengakses halaman detail data ikan
        $response = $this->actingAs($user)->get(route('dashboard.fishing-data.data-collection.show', $data->dtkn));
        
        // 6. berhasil melakukan penghapusan data ikan
        $response->assertSuccessful();
    }

    public function test_user_cannot_detail_data_collection()
    {
        ...
        // 2. sebuah akting bahwa pengguna telah melakukan login sebelumnya dan mengakses halaman detail data data ikan
        $response = $this->actingAs($user)->get(route('dashboard.fishing-data.data-collection.show', $data->id));
        
        // 3. kondisi dimana dilakukan pengecekkan role apakah pengguna seorang admin atau bukan
        if($user->role == 'admin')
        {
            // 4. jika pengguna adalah seorang admin maka akan diarahkan ke halaman data ikan
            $response->assertSuccessful();
            $response = $this->actingAs($user)->get(route('dashboard.fishing-data.data-collection.show', $data->dtkn));
        } else {
